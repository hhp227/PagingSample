...
internal class PagePresenter<T: Any>: NullPaddedList {
    ...
    /*private func dropPagesWithOffsets(_ pageOffsetsToDrop: ClosedRange<Int>) -> Int {
        var removeCount = 0
        var pageIterator = pages.makeIterator()
        var index = 0
        ...
        while let page = pageIterator.next() {
            if page.originalPageOffsets.contains(where: { pageOffsetsToDrop.contains($0) }) {
                removeCount += page.data.count
                pages.remove(at: index)
                pageIterator = pages.makeIterator()
                index -= 1
            }
            index += 1
        }
    ...
    private func dropPagesWithOffsets(_ pageOffsetsToDrop: ClosedRange<Int>) -> Int {
        ...
        pages.removeAll { page in
            page.originalPageOffsets.contains { pageOffsetsToDrop.contains($0) }
        }
        return removeCount
    }
