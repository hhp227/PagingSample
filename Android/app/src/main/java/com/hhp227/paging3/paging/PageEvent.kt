...
internal sealed class PageEvent<T : Any> {
    ...
    data class Insert<T : Any> private constructor(
        ...
        init {
            require(loadType == LoadType.APPEND || placeholdersBefore >= 0) {
                "Prepend insert defining placeholdersBefore must be > 0, but was" +
                        " $placeholdersBefore"
            }
            require(loadType == LoadType.PREPEND || placeholdersAfter >= 0) {
                "Append insert defining placeholdersAfter must be > 0, but was" +
                        " $placeholdersAfter"
            }
            require(loadType != LoadType.REFRESH || pages.isNotEmpty()) {
    ...
    data class Drop<T : Any>(
        ...
        init {
            require(loadType != LoadType.REFRESH) { "Drop load type must be PREPEND or APPEND" }
