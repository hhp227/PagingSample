...
// dropPagesWithOffsets확인 해볼것 요거 제외 원본과 동일
internal class PagePresenter<T : Any>(
    ...
    /*private fun dropPagesWithOffsets(pageOffsetsToDrop: IntRange): Int {
        var removeCount = 0
        val pageEnumerated = pages.withIndex()
    ...
    private fun dropPages(drop: PageEvent.Drop<T>, callback: ProcessPageEventCallback) {
        ...
        if (drop.loadType == LoadType.PREPEND) {
    ...
    internal companion object {
        private val INITIAL = PagePresenter<Any>(
            pages = mutableListOf(TransformablePage.EMPTY_INITIAL_PAGE),
            0,
            0
        )
