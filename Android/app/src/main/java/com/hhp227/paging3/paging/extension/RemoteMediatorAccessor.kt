...
private class AccessorState<Key : Any, Value : Any> {
    ...
    private val blockStates = Array(LoadType.values().size) {
    ...
    private fun computeLoadTypeState(loadType: LoadType): LoadState {
        ...
        return when (blockState) {
            BlockState.COMPLETED -> when (loadType) {
                LoadType.REFRESH -> LoadState.NotLoading(false)
                else -> LoadState.NotLoading(true)
            }
            BlockState.REQUIRES_REFRESH -> LoadState.NotLoading(false)
            BlockState.UNBLOCKED -> LoadState.NotLoading(false)
        }
    }
    ...
    fun getPendingBoundary() = pendingRequests.firstOrNull {
        it.loadType != LoadType.REFRESH && blockStates[it.loadType.ordinal] == BlockState.UNBLOCKED
    }?.let {
        // make a copy
    ...
    override fun requestRefreshIfAllowed(pagingState: PagingState<Key, Value>) {
        accessorState.use {
            if (it.refreshAllowed) {
                ...
                accessorState.requestLoad(LoadType.REFRESH, pagingState, ::launchRefresh, ::launchBoundary)
            }
        }
    }
